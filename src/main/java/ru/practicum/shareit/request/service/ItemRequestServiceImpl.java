...
public class ItemRequestServiceImpl implements ItemRequestService {
    ...
    private List<ItemRequestDto> addItemsToRequests(List<ItemRequestDto> requestsDtoList) {
        ...
        List<ItemDto> items = itemRepository.findByRequestsIds(requestsIds)
                .stream()
                .map(ItemMapper::toItemDto)
                .collect(Collectors.toList());
        Map<Long, ItemRequestDto> requestsMap = requestsDtoList.stream()
    ...
    public List<ItemRequestDto> getAllRequests(long userId, int from, int size) {
        ...
        List<ItemRequestDto> requestDtoList = itemRequestRepository.findAllPages(userId, pageRequest)
                .stream()
                .map(ItemRequestMapper::toItemRequestDto)
                .collect(Collectors.toList());
